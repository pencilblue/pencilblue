...
class Configuration {
  ...
  static load(filePaths) {
    if (util.isString(filePaths)) {
      filePaths = [filePaths]; // eslint-disable-line
    } else if (!filePaths) {
      filePaths = OVERRIDE_FILE_PATHS; // eslint-disable-line
    }
    ...
    for (let i = 0; i < filePaths.length; i += 1) {
      ...
      if (fs.existsSync(overrideFile)) {
        try {
          override = require(overrideFile); // eslint-disable-line
          overridesFound = true;
          break;
        } catch (e) {
  ...
  static mergeWithBase(overrides) {
    const multisite = overrides && overrides.multisite ? overrides.multisite.enabled : false;
    const baseConfig = Configuration.getBaseConfig(multisite);

    // merge in all overrides with the base configuration
    const config = util.deepMerge(overrides, baseConfig);
 ...
 * @type {String}
 */
const LOG_DIR = path.join(Configuration.DOCUMENT_ROOT, 'log');
 ...
 * @type {String}
 */
const LOG_FILE = path.join(LOG_DIR, 'pencilblue.log');
 ...
 * @type {String}
 */
const CONFIG_MODULE_NAME = 'config.js';
 ...
 */
Configuration.getBaseConfig = function getBaseConfig(multisite) {
  return {
    ...
    // the port is passed as an environment constiable.
    ...
    multisite: {
      ...
      // When multisite.enabled is true, this is the hostname that
      // will resolve to the global namespace.
    ...
    db: {
      ...
      // exist.
      indices: require('./dao/indices.js')(multisite), // eslint-disable-line
    },
    ...
    localization: {
      ...
      // The default locale is the fallback when localization
      // fails for the user's desired language.
      defaultLocale: 'en-US',
    },
    ...
    version: require(path.join(Configuration.DOCUMENT_ROOT, 'package.json')).version, //eslint-disable-line
  };
};
