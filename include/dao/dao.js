...
module.exports = function DAOModule(pb) {
    ...
    DAO.prototype.save = function(dbObj, options, cb) {
        ...
        this.getDb(function(err, db) {
            if (util.isError(err)) {
                if (pb.config.db.query_logging) {
                    pb.log.info(`err: ${JSON.stringify(err)}`);
                }
                return cb(err);
            }
            ...
            if (dbObj._id) {
                ...
                db.collection(dbObj.object_type).updateOne({ _id: dbObj._id }, { $set: dbObj }, options, function(err/*, writeOpResult*/) {
                    ...
                    if (!err && pb.config.db.query_logging) {
                        pb.log.info(`updated successfully, options: ${JSON.stringify(options)}`);
                    }
                    if (err && pb.config.db.query_logging) {
                        pb.log.info(`err with update: ${JSON.stringify(err)}`);
                    }
                });
            } else {
    ...
    DAO.prototype.ensureIndex = function(procedure, cb) {
        ...
        pb.dbm.getDb(this.dbName, function(err, db) {
            ...
            db.collection(collection).createIndex(spec, options, cb);
        });
    };
