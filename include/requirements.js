...
module.exports = function PB(config) {
  // older versions of node did not have process as a core module.
  // We ensure that we have access to it here and ensure
  // it is set as a global constiable
  if (!global.process) {
  ...
  // configure cache
  const CacheModule = require(path.join(config.docRoot, '/include/dao/cache.js'));
  ...
  const ValidationModule = require(path.join(config.docRoot, '/include/validation/validation_service.js'));
  ...
  const SessionModule = require(path.join(config.docRoot, '/include/session/session.js'));
  ...
  const TemplateModule = require(`${config.docRoot}/include/service/entities/template_service.js`)(pb);
  ...
  const Authentication = require(path.join(config.docRoot, '/include/security/authentication'))(pb);
  ...
  const BodyParsers = require(path.join(config.docRoot, 'include/http/parsers'))(pb);
  ...
  pb.settings = pb.SettingServiceFactory.getService(
    pb.config.settings.use_memory,
    pb.config.settings.use_cache);
  ...
  const ArticleServiceModule = require(path.join(config.docRoot, '/include/service/entities/article_service.js'))(pb);
