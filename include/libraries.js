...
module.exports = function LibrariesServiceModule(pb) {
   ...
   * @type {String}
   */
  const LIBRARIES_SETTINGS_REF = 'libraries_settings';
   ...
   * @type {Object}
   */
  const CDN_DEFAULTS = Object.freeze({
   ...
   * @type {Object}
   */
  const BOWER_DEFAULTS = Object.freeze({
  ...
  class LibrariesService {
    ...
    constructor() {
       ...
       */
      this.getSettings = (cb) => {
        pb.settings.get(LIBRARIES_SETTINGS_REF, (err, settings) => {
          if (settings) {
            return cb(err, settings);
          }

          // set default settings if they don't exist
          settings = LibrariesService.getCDNDefaults(); // eslint-disable-line no-param-reassign
          return pb.settings.set(LIBRARIES_SETTINGS_REF, settings, (setErr) => {
            cb(setErr, settings);
          });
        });
      };
     ...
     */
    static init(cb) {
      const instance = new LibrariesService();
      instance.getSettings((err, settings) => {
        if (util.isError(err)) {
          return cb(err);
        }
