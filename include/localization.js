...
module.exports = function LocalizationModule(pb) {
   ...
   * @return {Object}
   */
  function parseLocale(locale, parseLocaleStr) {
    if (util.isString(locale)) {
      locale = parseLocaleStr(locale); // eslint-disable-line no-param-reassign
    }
   ...
   */
  function findKeyBlock(key, create, storage) {
    if (create && typeof storage[key] === 'undefined') {
      storage[key] = {}; // eslint-disable-line no-param-reassign
    }
    return storage[key];
  }
  /**
  ...
  class Localization {
    ...
    localize(sets, text, hostname) {
      ...
      const loc = Localization.storage;
      if (util.isNullOrUndefined(loc)) {
        throw new Error(`Failed to set a language. LANG=${util.inspect(this.language)}`);
      }

      // to squeeze a little performance out of an outdated function we convert the
      // sets to a quick lookup to reduce
      // the number of comparisons necessary to see if we should even check for a localization
      const self = this;
      const superSets = ['generic'];
      util.arrayPushAll(sets, superSets);
      const setLookup = superSets.reduce((lookup, prefix) => {
        lookup[prefix] = true;  // eslint-disable-line no-param-reassign
        return lookup;
      }, {});

      text = Object.keys(Localization.storage) // eslint-disable-line no-param-reassign
        .reduce((content, key) => {
          // look to see if the set even matches
          const pieces = key.split('.');

          // see if the set matches only when the set is in the lookup and
          // when the key is nested (backward compatibility piece)
          if (pieces.length === 1 || !setLookup[pieces[0]]) {
            return content;
          }

          const setVal = self.g(key /* , {empty options}*/);
      ...
      if (text.indexOf('<body') >= 0) {
        text = text  // eslint-disable-line no-param-reassign
          .concat(pb.ClientJs.includeJS(pb.UrlService.createSystemUrl(
            `api/localization/script?locale=${this.language}`, { hostname })));
      }
    ...
    /**
     * Translates a single key.  The function accepts a constiable number of
     ...
     * @method get
     * @param {String} key
     * @param {String|Integer|Number|Object} [args] The constiable number of
    ...
    get(...args) {
      ...
      if (util.isNullOrUndefined(Localization.storage[key])) {
        // now we must loop over the keys to see if we were given only the
        // second half of a localization key
        // this is extremely inefficient and hence why the function is deprecated
        const keys = Object.keys(Localization.storage);
        for (let i = 0; i < keys.length; i += 1) {
          // check to see if the the key is partitioned
          const index = keys[i].indexOf('.');
          if (index < 0) {
      ...
      let val = this.g(convertedKey /* , {empty options}*/);
      if (val !== null) {
        args[0] = val; // eslint-disable-line no-param-reassign
        val = util.format(...args);
      }
      return val;
    }
    ...
    g(key, options) {
      ...
      // TODO retrieve active plugins for site to narrow down which
      // plugins should be examined during retrieval

      // get the current local as object
      let locale = this.localeObj;

      // get theme to prioritize
      const plugin = options.plugin || this.activeTheme;

      // define convenience functions
      const self = this;
      const processValue = (localization) => {
        // set cache
      ...
      const processKeyBlock = (keyBlock) => {
        ...
        if (!util.isNullOrUndefined(keyBlock.__plugins)) {
          ...
          const pluginsToInspect = Object.keys(pluginsBlock);
          for (let j = 0; j < pluginsToInspect.length; j += 1) {
      ...
      const processLanguageBlock = function (langBlock) {
        ...
        if (util.isString(locale.countryCode)) {
          const countryKey = k(locale.countryCode);
          if (!util.isNullOrUndefined(langBlock[countryKey])) {
            const countryResult = processKeyBlock(langBlock[countryKey]);
        ...
        const langResult = processKeyBlock(langBlock);
      ...
      // verify that key even exists, if not we're done.
      // Just send back the default val, if provided
      ...
      // key create key path
      const keyBlock = findKeyBlock(key, false, Localization.storage);
      if (!keyBlock) {
        return null;
      }
      ...
      // value to pick
      let langKey = k(locale.language);
      let result = processLanguageBlock(keyBlock[langKey]);
      if (!util.isString(result)) {
        // check to see if we should fall back to the default locale
        const defaultLocale = Localization.parseLocaleStr(Localization.getDefaultLocale());
        if (defaultLocale.language !==
            this.localeObj.language ||
            defaultLocale.countryCode !==
            this.localeObj.countryCode) {
    ...
    static best(request, supported) {
      supported = util.isArray(supported) ?  // eslint-disable-line no-param-reassign
        new Locale.Locales(supported) : Localization.supported;

      let locales;
      let loc = Localization.getDefaultLocale();
      if (request) {
        const acceptLangStr = util.isString(request) ? request :
    ...
    static init(cb) {
      ...
      const options = {
        recursive: false,
        filter: filePath => filePath.indexOf(JS_EXT) === filePath.length - JS_EXT.length,
      };
      const localizationDir = path.join(pb.config.docRoot, 'public', 'localization');
      util.getFiles(localizationDir, options, (err, files) => {
        if (util.isError(err)) {
          return cb(err);
        }

        let compoundedResult = true;
        files.forEach((file) => {
          // parse the file
          let obj = null;
          try {
            obj = require(file); // eslint-disable-line global-require, import/no-dynamic-require
          } catch (e) {
          ...
          const localeObj = Localization.parseLocaleStr(file);
    ...
    static isSupported(locale) {
      ...
      const localeObj = Localization.parseLocaleStr(locale);
      locale = Localization.formatLocale(  // eslint-disable-line no-param-reassign
        localeObj.language,
        localeObj.countryCode);
    ...
    static getLocalizationPackage(locale, options) {
      ...
      const ls = new Localization(locale);
      const packageObj = {};
      Object.keys(Localization.storage).forEach((key) => {
        const result = ls.g(key, options);

        const parts = key.split(Localization.KEY_SEP);
        if (parts.length === 1) {
          packageObj[key] = result;
          return;
        }

        let block = packageObj;
     ...
     * @return {Boolean}
     */
    static registerLocalizations(locale, localizations, options) {
    ...
    static _registerLocale(locale, localizations, options) {
      locale = parseLocale( // eslint-disable-line no-param-reassign
        locale,
        Localization.parseLocaleStr);
      ...
      const queue = [];
      const processObj = function (prefix, obj) {
      ...
      while (queue.length > 0) {
        ...
        if (util.isObject(item.val)) {
          ...
          compoundedResult = compoundedResult &&
            Localization._registerLocalization(locale, item.key, item.val, options);
        } else {
      ...
      if (compoundedResult && !Localization.isSupported(locale)) {
        Localization.supportedLookup[Localization.formatLocale(
          locale.language,
    ...
    /**
     * Registers a single localization key for the provided locale.
     *  Optionally, the localization can be scoped to a single plugin.
     * @private
     * @static
     * @method _registerLocalization
     * @param {String|object} locale
    ...
    static _registerLocalization(locale, key, value, options) {
      ...
      const keyBlock = findKeyBlock(key, true, Localization.storage);

      // ensure that the language block exists
      const langKey = k(locale.language);
      if (util.isNullOrUndefined(keyBlock[langKey])) {
        keyBlock[langKey] = {};
      }
      let insertionBlock = keyBlock[langKey];
    ...
    static unregisterLocale(locale, options) {
      ...
      const keysRemoved = Object.keys(Localization.storage).reduce((prev, key) =>
      prev + Localization.unregisterLocalization(locale, key, options), 0);

      // remove from quick lookup
      delete Localization.supportedLookup[Localization.formatLocale(
        locale.language,
        locale.countryCode)];
      Localization.supported = new Locale.Locales(Object.keys(Localization.supportedLookup));
    ...
    static unregisterLocalization(locale, key, options) {
      ...
      const langKey = k(locale.language);
      const langBlock = keyBlock[langKey];
      ...
      if (util.isString(locale.countryCode)) {
        ...
        if (!util.isNullOrUndefined(langBlock[countryKey])) {
          ...
          const countryBlock = langBlock[countryKey];// translate to plugin key
          if (util.isString(options.plugin)) {
            // the plugin was specified so we should check the country
            // code block for a sub-section for the plugin
    ...
    static replaceParameters(value, params, defaultVal) {
      ...
      for (let i = 0; i < value.length; i += 1) {
        if (!isOpen && value[i] === '{' && prev !== '%') {
          ...
          val += params[constiable] || defaultVal || constiable;
          isOpen = false;
          constiable = '';
        } else if (isOpen) {
          constiable += value[i];
    ...
    static getSupportedWithDisplay() {
      const supported = Localization.getSupported();
    ...
    static parseLocaleStr(filePath) {
      ...
      let extPos = filePath.lastIndexOf(JS_EXT);
      if (extPos < 0) {
        extPos = filePath.length;
      }
      const locale = filePath.substr(lastSlashPos + 1, extPos - lastSlashPos - 1);

      const parts = locale.split(Localization.LOCALE_PART_SEP);
      const lang = parts[0] ? parts[0].toLowerCase() : null;
      const countryCode = parts[1] ? parts[1].toUpperCase() : null;
      return {
        language: lang,
        countryCode,
      };
    ...
    static formatLocale(language, countryCode) {
      ...
      let localeStr = language.toLowerCase();
