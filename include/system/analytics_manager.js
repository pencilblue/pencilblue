...
module.exports = function AnalyticsManagerModule(pb) {
    ...
    AnalyticsManager.registerProvider = function(name, site, onPageRendering) {
        ...
        if (!pb.validation.validateNonEmptyStr(name) || AnalyticsManager.isRegistered(name, site) || !util.isFunction(onPageRendering)) {
            return false;
        }
    ...
    AnalyticsManager.isRegistered = function(name, site) {
        if (!site) { site = pb.SiteService.GLOBAL_SITE; }
        return !(util.isNullOrUndefined(PROVIDER_HOOKS[site]) || util.isNullOrUndefined(PROVIDER_HOOKS[site][name]));
    };
    ...
    AnalyticsManager.onPageRender = function(req, session, ls, cb) {
        ...
        var managerInstance = new AnalyticsManager({ site: site });
