...
RedisCommandBroker.prototype.shutdown = function(cb) {
    var clients = [this.publishClient, this.subscribeClient];
...
RedisCommandBroker.prototype.publish(command) {

};

RedisCommandBroker.prototype.subscribe(channel, onCommandReceived,cb) {
    this.subscribeClient.once('subscribe', function(channel, count) {
        cb(null, count);
    });
