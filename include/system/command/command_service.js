...
var ServerRegistration = require('');
...
CommandService.prototype.sendCommandGetResponse = function (type, options, onResponse) {
    ...
    var doSend = function (type, options, onResponse) {
        ...
        var timeout = options.timeout || Configuration.active.command.timeout || DEFAULT_TIMEOUT;
        var d = domain.create();
...
CommandService.getInstance = function () {
    ...
    if (DEFAULT_BROKERS[config.command.broker]) {
        CommandBroker = DEFAULT_BROKERS[config.command.broker];
    }
    else {
